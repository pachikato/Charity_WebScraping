...
class ReadyforSpider(scrapy.Spider):
    ...
    def parse_project(self,response):
        ...
        for i in range(1, len(response.xpath("//section[@class='Project-return Side-area-1 u-mb_20']").extract())+1):
            menulist = "//section[@class='Project-return Side-area-1 u-mb_20'][%s]" % i
            donation = response.xpath(menulist + "/descendant::span[@class='Project-return__price']/text()").extract()
